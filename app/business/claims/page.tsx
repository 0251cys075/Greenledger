...
function VerdictBadge({ verdict }: { verdict?: string }) {
  if (!verdict) return <span className="text-xs text-[#718875] bg-[#EFECE4] px-2.5 py-1 rounded-full">Pending</span>;
  if (verdict === 'VERIFIED')
...
function EvidenceStrengthBar({ strength }: { strength?: string }) {
  const map: Record<string, { width: string; color: string }> = {
    STRONG: { width: 'w-full', color: 'bg-[#3E7D4F]' },
    MODERATE: { width: 'w-2/3', color: 'bg-[#C9A227]' },
    WEAK: { width: 'w-1/3', color: 'bg-[#C1443E]' },
    NONE: { width: 'w-0', color: 'bg-[#D6D3C8]' },
  };
  const cfg = map[strength || 'NONE'] || map.NONE;
  return (
    <div className="flex items-center gap-2">
      <div className="flex-1 h-1.5 bg-[#EFECE4] rounded-full overflow-hidden">
        <div className={`h-full rounded-full transition-all ${cfg.width} ${cfg.color}`} />
      </div>
      <span className="text-xs text-[#718875] w-16 text-right">{strength?.replace('_', ' ') || 'None'}</span>
    </div>
  );
}
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-[#1C1C1C]">Environmental Claims</h1>
          <p className="text-sm text-[#718875] mt-0.5">
      ...
      <div className="flex items-center gap-2 mb-6 overflow-x-auto pb-1">
        <Filter size={14} className="text-[#718875] shrink-0" />
        {tabs.map((tab) => (
          <button
            key={tab.value}
            ...
            className={`shrink-0 px-4 py-1.5 rounded-full text-sm font-medium transition-all ${
              filter === tab.value
                ? 'bg-[#1B3A2B] text-[#F7F5F0]'
                : 'bg-[#EFECE4] text-[#718875] hover:bg-[#D6D3C8]'
            }`}
          >
            {tab.emoji} {tab.label}
      ...
      {loading ? (
        <div className="card-cream overflow-hidden">
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="p-5 border-b border-[#D6D3C8] animate-pulse flex gap-4">
              <div className="flex-1 h-4 bg-[#EFECE4] rounded" />
              <div className="w-28 h-7 bg-[#EFECE4] rounded-full" />
            </div>
          ))}
        </div>
      ) : filtered.length === 0 ? (
        <div className="card-cream p-16 text-center">
          <FileText size={36} className="text-[#D6D3C8] mx-auto mb-4" />
        ...
        <div className="card-cream overflow-hidden">
          {/* Table Header */}
          <div className="hidden md:grid grid-cols-[1fr_140px_140px_110px] gap-4 px-6 py-3 bg-[#EFECE4] border-b border-[#D6D3C8]">
            <p className="text-xs font-semibold text-[#718875] uppercase tracking-wider">Claim</p>
            <p className="text-xs font-semibold text-[#718875] uppercase tracking-wider">Evidence</p>
            <p className="text-xs font-semibold text-[#718875] uppercase tracking-wider">Verdict</p>
            <p className="text-xs font-semibold text-[#718875] uppercase tracking-wider">Actions</p>
          </div>

          {/* Table Rows */}
          <div className="divide-y divide-[#D6D3C8]">
            {filtered.map((claim) => (
              <div key={claim.id} className="px-6 py-5 hover:bg-[#FCFAF5] transition-colors">
                <div className="md:grid md:grid-cols-[1fr_140px_140px_110px] md:gap-4 md:items-center">
                  {/* Claim Text */}
                  <div className="mb-3 md:mb-0">
                    <p className="text-xs font-semibold text-[#A9BBA0] uppercase tracking-wider mb-1">
                      {claim.product_name}
                      {claim.is_demo && <span className="ml-2 text-[#C9A227] opacity-75">· DEMO</span>}
                    </p>
                  ...
                  {/* Evidence Strength */}
                  <div className="mb-3 md:mb-0">
                    <p className="text-xs text-[#718875] mb-1.5 md:hidden font-semibold">Evidence</p>
                    <EvidenceStrengthBar strength={claim.evidence_strength} />
                    <p className="text-xs text-[#A8B3AA] mt-1">
                  ...
                  {/* Verdict */}
                  <div className="mb-3 md:mb-0">
                    <p className="text-xs text-[#718875] mb-1.5 md:hidden font-semibold">Verdict</p>
                  ...
                  <div className="flex flex-col gap-1.5">
                    <Link
                      ...
                      className="flex items-center gap-1 text-xs text-[#A9BBA0] font-semibold hover:underline"
                    >
                      View Record <ExternalLink size={10} />
                    </Link>
                    {claim.verdict === 'INSUFFICIENT_EVIDENCE' && (
                      <Link href="/business/claims/new" className="text-xs text-[#C9A227] hover:underline">
                        Add Evidence
                      </Link>
                    )}
                  </div>
